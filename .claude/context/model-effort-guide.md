...
| 대상                             | 모델         | Effort   |
| -------------------------------- | ------------ | -------- |
| `/dev-kickoff` (optional)        | sonnet-4-6   | high     |
| `/dev-open`                      | sonnet-4-6   | high     |
| `/dev-sprint`                    | **opus-4-6** | **high** |
| `/dev-close`                     | sonnet-4-6   | low      |
| `/dev-check` (optional)          | sonnet-4-6   | normal   |
...
다음 조건을 모두 충족하면 `/dev-sprint` 배치에서도 sonnet-4-6을 써도 된다:
...
### `/dev-kickoff` — sonnet-4-6 + high

프로젝트 상태 분석 후 개발건 후보를 도출하여 dev-list에 추가.
dev-open과 같은 수준의 구조화된 판단 필요. 복잡한 추론보다 상태 파악과 분류 중심.
개발건 도출 결과가 이후 세션 방향을 결정하므로 effort는 high.

### `/dev-open` — sonnet-4-6 + high

dev-list 기반 스프린트 계획 고정.
...
docs-sync(필요 시)와 handoff 정리, dev-list 정리를 한 번에 처리.
코드·결정의 문서 반영은 기계적 작업, handoff는 요약·조직화 중심.
복잡한 추론 없이 빠르게 끝내는 게 목적.

### `/dev-check` (optional) — sonnet-4-6 + normal

`dev-open` 계획과 현재 상태 대조, 남은 스프린트 순서/범위 재조정 제안.
스프린트 3개 이상이거나 예상 밖 상황이 생겼을 때만 사용.
...
**`packages/shared`**
포맷팅 유틸, 상수, 에러 타입, 설정 스키마/해석 포함. 순수 함수 위주, 의존성 없음.
