...
| 대상                             | 모델         | Effort   |
| -------------------------------- | ------------ | -------- |
| `/kick-off`                      | sonnet-4-6   | normal   |
| `/session-start`                 | sonnet-4-6   | high     |
| `/work`                          | **opus-4-6** | **high** |
| `/docs-sync`                     | sonnet-4-6   | normal   |
| `/close-session`                 | sonnet-4-6   | low      |
...
다음 조건을 모두 충족하면 `/work` 배치에서도 sonnet-4-6을 써도 된다:
...
코드베이스 스캔 후 작업 후보 1~3개를 제안하는 PM 역할.
창의적 설계보다 넓은 맥락 파악이 핵심이라 Opus 없이 sonnet으로 충분.
속도 이점이 체감된다.

### `/session-start` — sonnet-4-6 + high

목표·범위·stop line을 고정하는 단계.
이 결정이 틀리면 배치 전체가 낭비되므로 effort는 high로.
복잡한 추론보다 구조화된 판단이 필요해 sonnet이 적합.
...
작성된 코드·결정을 docs, context, registry, log에 반영하는 기계적 작업.
일관성이 중요하고 창의적 판단은 거의 불필요. Opus 낭비 없이 sonnet으로 충분.

### `/close-session` — sonnet-4-6 + low

handoff 정리, scratch 갱신. 요약·조직화 중심.
