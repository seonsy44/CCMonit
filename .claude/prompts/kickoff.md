...
- 작업 범위를 먼저 좁히고, 어떤 skill을 사용할지 제안한다.
- 바로 구현을 길게 진행하지 말고, 먼저 작업 단위와 읽을 파일을 정리한다.

## 작업 원칙
- docs를 source of truth로 본다.
- 작은 배치로 작업한다.
- 완료 후 `.claude/scratch/worklog.md` 와 `next-prompt.md`를 갱신할 수 있게 정리한다.
- 관련 docs 변경이 필요하면 같이 표시한다.

## 원하는 출력
1. 이번 세션에 가장 적합한 작은 작업 1개
2. 읽어야 할 파일 목록
3. 사용할 skill 제안
4. 완료 조건
