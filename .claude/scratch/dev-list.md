...
도메인 엔티티 7개 모두 완성(2026-04-12). CLI 부트스트랩 완료로 `pnpm dev` TUI 실행 가능. 다음 레이어는 도메인 서비스 → 애플리케이션 포트/유스케이스 → 인프라 어댑터 → TUI 데이터 연결 순이다. `.claude/` 운영 레이어 토큰 최적화 완료(2026-04-12): workflow.md 159줄, 스킬 읽기 효율화.
