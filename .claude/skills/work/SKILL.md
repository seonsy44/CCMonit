...
2. 불확실성이 높고 범위가 크면 내부적으로 한 단계 줄인다. 불확실성이 낮고 패턴이 반복되는 경우에는 session-start가 정한 범위를 유지한다.
3. 실제 코드 또는 문서 작업을 수행한다.
4. session-start가 복수 배치를 계획한 경우, 첫 배치 완료 후 다음 배치로 자연스럽게 이어간다.
5. 목표 대비 과도한 수정이 없는지 짧게 확인한다.
6. `docs-sync`가 필요한지 여부를 마지막에 표시한다.
