---
name: session-start
description: 짧은 세션을 시작할 때 목표, 제약, 이번 턴의 종료선을 고정한다. 저렴한 요금제에서 작업을 잘게 쪼개기 위한 시작 스킬.
...
- 다음 세션이 자연스럽게 이어질 수 있게 문맥 손실을 줄인다.

## 수행 절차
1. `docs/index.md`, `docs/registry/page-index.md`, `.claude/context/*.md`, 필요 시 `.claude/skills/_shared/*.md`, `.claude/scratch/*`를 읽고 현재 프로젝트 문맥을 8줄 이내로 요약한다.
