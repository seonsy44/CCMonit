...
### 0단계: user-req.md 확인 및 interview (조건부)

`true` 파라미터가 있을 때만 실행한다.

1. `.claude/scratch/user-req.md`를 읽는다.
2. 구분선(`---`) 아래에 실질적 내용이 없으면 중단:
   - "`.claude/scratch/user-req.md`에 요구사항을 적고 다시 `/dev-kickoff true`를 실행해주세요."
3. 내용이 있지만 모호한 경우 interview:
   - 모호함 판단 기준:
     - 대상이 불명확하거나 프로젝트 내 대응 범위가 3개 패키지 이상에 걸치는 경우
     - 요구사항이 상충하거나 해석이 2가지 이상인 경우
     - "잘", "빨리", "좋게" 같은 방향만 있고 구체 대상이 없는 경우
   - 1~2개 질문만 한다 (선택지 형태 권장)
   - 답변 후 1단계로 진행한다
4. 내용이 명확하면 바로 1단계로 간다.
