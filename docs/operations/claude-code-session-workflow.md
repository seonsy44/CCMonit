...
## 1. 핵심 원칙
- 한 세션에 목표는 1개만 둔다. 목표의 범위는 작업 성격에 따라 조절한다.
- 스킬 수를 최소화한다.
...
- 세레모니(읽기/계획/정리) 대비 실제 작업 비율이 합리적이어야 한다. 세레모니가 산출물보다 명백히 크면 범위를 넓히거나 세레모니를 줄인다.

## 1-1. 세션 범위 판단 기준

세션 범위는 **불확실성**과 **패턴 반복도** 두 축으로 판단한다.
